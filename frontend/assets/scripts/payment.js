  ...
  if (payBtn) {
    payBtn.addEventListener("click", function (e) {
      ...
      if (!selectedPayment) {
        ...
        const msg =
          typeof getPaymentTranslation === "function"
            ? getPaymentTranslation("selectPaymentMethod", lang)
...
function validateCardForm() {
  ...
  if (!cardNumber || cardNumber.length < 13) {
    ...
    const msg =
      typeof getPaymentTranslation === "function"
        ? getPaymentTranslation("invalidCardNumber", lang)
  ...
  if (!expiryDate || !expiryDate.match(/^\d{2}\/\d{2}$/)) {
    ...
    const msg =
      typeof getPaymentTranslation === "function"
        ? getPaymentTranslation("invalidExpiryDate", lang)
  ...
  if (!cvv || cvv.length < 3) {
    ...
    const msg =
      typeof getPaymentTranslation === "function"
        ? getPaymentTranslation("invalidCVV", lang)
        : "Vui lòng nhập mã CVV hợp lệ";
    notify(msg, "info", 4000);
    return false;
  }
  ...
  if (!cardName.trim()) {
    ...
    const msg =
      typeof getPaymentTranslation === "function"
        ? getPaymentTranslation("invalidCardName", lang)
...
function processEWalletPayment() {
  ...
  const msg =
    typeof getPaymentTranslation === "function"
      ? getPaymentTranslation("selectEwallet", lang)
...
function showLoadingState() {
  ...
  const text =
    typeof getPaymentTranslation === "function"
      ? getPaymentTranslation("processing", lang)
      : "Đang xử lý...";
...
function hideLoadingState() {
  ...
  const text =
    typeof getPaymentTranslation === "function"
      ? getPaymentTranslation("payNow", lang)
      : "Thanh toán ngay";
  payBtn.innerHTML = iconHtml + "<span>" + text + "</span>";
}
...
function processVNPayPayment() {
  ...
  btnElement.innerHTML =
    '<i class="fas fa-spinner fa-spin"></i><span>Đang kết nối VNPay...</span>';
  ...
  const apiEndpoint =
    window.SkyPlanConfig?.getApiEndpoints().vnpayCreate ||
  ...
  fetch(apiEndpoint, {
    ...
    .catch((error) => {
      ...
      const prefix =
        typeof getPaymentTranslation === "function"
          ? getPaymentTranslation("errorPrefix", lang)
          : "Lỗi: ";
...
function simulateVNPayTest(paymentData) {
  ...
  window.addEventListener("message", function (event) {
    if (event.data.type === "vnpay_success") {
      ...
      // Reset button
      btnElement.innerHTML =
        '<i class="fas fa-credit-card"></i><span>Thanh toán với VNPay</span>';
      btnElement.disabled = false;
    }
  });
}
...
function updatePayBtnUI() {
  ...
  if (selectedPayment.value === "card") {
    ...
    if (mainPayBtnText) {
      mainPayBtnText.textContent =
        typeof getPaymentTranslation === "function"
          ? getPaymentTranslation("bankConfirm", lang)
          : "Đến trang xác nhận";
    }
  ...
  ["cardNumber", "expiryDate", "cvv", "cardName"].forEach((id) => {
    ...
    if (el)
      el.addEventListener("input", function () {
        try {
          if (typeof window.updatePayBtn === "function") window.updatePayBtn();
        } catch (e) {}
      });
  });
});
...
// --- Extras integration into order summary ---
function updateSummaryWithExtras() {
  try {
    const summary = document.querySelector(".price-breakdown");
    if (!summary) return;
    let state = null;
    try {
      state = JSON.parse(localStorage.getItem("skyplan_extras_v2"));
    } catch (e) {}
    const extrasTotal =
      state && typeof state.total === "number" ? state.total : 0;
    if (!extrasTotal || extrasTotal <= 0) return;

    const lang =
      localStorage.getItem("preferredLanguage") ||
      document.documentElement.lang ||
      "vi";
    const label = lang === "vi" ? "Dịch vụ thêm" : "Extras";
    const fmt = (n) => (n || 0).toLocaleString("vi-VN") + " VND";

    // Insert or update extras row
    let extrasRow = summary.querySelector(".price-item.extras");
    if (!extrasRow) {
      extrasRow = document.createElement("div");
      extrasRow.className = "price-item extras";
      const nameSpan = document.createElement("span");
      nameSpan.textContent = label;
      const valSpan = document.createElement("span");
      valSpan.className = "extras-amount";
      valSpan.textContent = fmt(extrasTotal);
      extrasRow.appendChild(nameSpan);
      extrasRow.appendChild(valSpan);
      // Insert before total row if exists
      const totalRow = summary.querySelector(".price-item.total");
      if (totalRow && totalRow.parentNode === summary) {
        summary.insertBefore(extrasRow, totalRow);
      } else {
        summary.appendChild(extrasRow);
      }
    } else {
      const valSpan =
        extrasRow.querySelector(".extras-amount") ||
        extrasRow.querySelector("span:last-child");
      if (valSpan) valSpan.textContent = fmt(extrasTotal);
    }

    // Update grand total
    const totalRow = summary.querySelector(".price-item.total");
    if (totalRow) {
      const val = totalRow.querySelector("span:last-child");
      if (val) {
        const base =
          parseInt((val.textContent || "0").replace(/[^0-9]/g, ""), 10) || 0;
        val.textContent = fmt(base + extrasTotal);
      }
    }
  } catch (e) {
    // swallow errors to avoid breaking payment page
  }
}
